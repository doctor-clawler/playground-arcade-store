...
export const COIN_SPAWN_INTERVAL = 4;
...
function coinPositionFor(index, arena) {
  ...
  return {
    x: 96 + ((index * 263) % (arena.width - 192)),
    y: 126 + ((index * 157) % (arena.height - 252)),
  };
}

function spawnCoin(next) {
  const position = coinPositionFor(next.run.coinsSpawned, next.arena);
  next.coins.push({
    id: `coin-${next.run.coinsSpawned}`,
    x: position.x,
    y: position.y,
    r: 26,
    value: COIN_VALUE,
  });
  next.run.coinsSpawned += 1;
}

function spawnStartingCoins(next) {
  while (next.coins.length < STARTING_COINS) spawnCoin(next);
}

function spawnDueCoins(next) {
  while (next.run.nextCoinAt <= next.run.elapsed && next.coins.length < MAX_COINS) {
    spawnCoin(next);
    next.run.nextCoinAt += COIN_SPAWN_INTERVAL;
  }
}

function collectNearbyCoins(next) {
  const remaining = [];
  for (const coin of next.coins) {
    const closeEnough = distance(next.player, coin) <= next.player.r + coin.r + 14;
    if (closeEnough) {
      next.money += coin.value;
      next.run.coinsCollected += 1;
    } else {
      remaining.push(coin);
    }
  }
  next.coins = remaining;
}
...
export function createInitialState({ money = 0, inventory = {} } = {}) {
  return {
    ...
    run: {
      ...
      coinsSpawned: 0,
      coinsCollected: 0,
      nextCoinAt: COIN_SPAWN_INTERVAL,
    },
    player: {
      x: ARENA.width / 2,
      y: ARENA.height / 2,
