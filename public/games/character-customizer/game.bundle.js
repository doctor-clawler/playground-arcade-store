...
function createInitialState(savedCharacter = null) {
...
function startCustomizing(state) {
...
function exitWithoutSaving(state) {
  state.mode = "start";
  state.draft = cloneCharacter(state.saved || createDefaultCharacter());
  state.activeTab = "skin";
  state.modeStartedAt = state.time;
...
function setSkinColor(state, color) {
  if (allowedSkinColors.has(color)) state.draft.skinColor = color;
  return state;
}

function selectOutfit(state, outfit) {
...
function activate(id) {
  ...
  if (id === "start") {
    ...
    saveAndExit(state);
    safeStorage.setItem(STORAGE_KEY, JSON.stringify(state.saved));
